...
{
	if(pArrayListEmployee != NULL && !(ll_sort(pArrayListEmployee, employee_comparaPorNombre, 0)) && ll_len(pArrayListEmployee)>0)
	{
		printf("\nOrdenado por Nombre\n");
	}
	else{
...
/** \brief Aumenta los sueldos usando ll_Map
 *
 * 	\return int -1 si error, 0 si fue exitosa
 *
 */
int controller_aumentarSueldos(LinkedList* listaEmpleados)
{
	int retorno = -1;

	if(listaEmpleados != NULL && ll_len(listaEmpleados)>0)
	{
		if(!ll_map(listaEmpleados, aumentoSueldo))
				{
			printf("\nAmuento aplicado\n");
				}
	}
...
/** \brief Filtra la lista usando ll_filter
 *
 * 	\return int -1 si error, 0 si fue exitosa
 *
 */
int controller_Filtrar( LinkedList* listaEmpleados)
{
	LinkedList* listaAux;
	int retorno = -1;
	if(listaEmpleados != NULL && ll_len(listaEmpleados)>0 	)
	{
		retorno = 0;
		listaAux = ll_filter(listaEmpleados, filtroHoras);
...
/** \brief Cuenta la cantidad de elementos que cumplan con el parámetro de la función
 * 	       usando la función ll_reduce.
 *
 * 	\return int -1 si error, 0 si fue exitosa
 *
 */
int controller_Contar(LinkedList* listaEmpleados)
{
		...
		{
			retorno = 0;
			resultado = ll_reduce(listaEmpleados, contarPorHoras);
			printf("\nLa cantidad de Empleados con 100 o más horas es: %d", resultado);
		}
