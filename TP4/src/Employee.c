...
/** \brief Verifica que el Elemento cumpla con el parámetro asignado
 *
 * \param void* pElemento, puntero al elemento a verificar
 * \return retorna 1 si tuvo exito - (-1) si hay error.
 *
 */
int aumentoSueldo(void* pElemento)
{
	int retornoAux = -1;
	int horasAux;
	int sueldoAux;
	char sueldoChar[256];
	Employee * elemento = (Employee*) pElemento;

	if(elemento!= NULL && !employee_getHorasTrabajadas(elemento, &horasAux) && !employee_getSueldo(elemento, &sueldoAux))
	{
		retornoAux = 0;
		if(horasAux >= 100)
		{
					sueldoAux = sueldoAux + 9999;
					snprintf(sueldoChar, sizeof(sueldoChar), "%d", sueldoAux);
					employee_setSueldo(elemento, sueldoChar);
		}
	}
	return retornoAux;
}
 ...
 *
 */
int filtroHoras(void* pElemento)
{
	int retornoAux = -1;
	int horasAux;
	Employee* elemento = (Employee*)pElemento;
	if(elemento!= NULL && !employee_getHorasTrabajadas(elemento, & horasAux))
	{
		if(horasAux >= 100)
		{
			retornoAux = 0;
 ...
 *
 */
int contarPorHoras(void* pElemento)
{
	int retornoAux = -1;
	int horasAux;
	Employee* elemento = (Employee*)pElemento;
	if(elemento!= NULL && !employee_getHorasTrabajadas(elemento, & horasAux) && horasAux >= 100)
	{
	retornoAux = 0;
	}
